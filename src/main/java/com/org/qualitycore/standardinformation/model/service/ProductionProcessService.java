    ...
    public StandardInformationMessage createMaterialGrinding(MaterialGrindingDTO materialGrindingDTO) {
        try {
            ...
            if(materialGrinding.getGrindDuration()==null){
                throw new IllegalArgumentException("분쇄 소요시간이 입력이 되었어 ");
            }
            ...
            //DB 저장
            MaterialGrinding saveMaterialGrinding = productionProcessRepository.save(materialGrinding);
            log.info("서비스 분쇄 공정 등록 완료 ! {}", saveMaterialGrinding);

            return new StandardInformationMessage(HttpStatus.CREATED.value(), "분쇄공정 등록 완료!");

        } catch(IllegalArgumentException e){
            log.error("서비스 : 입력값 오류 발생 {}" , e.getMessage(), e);
